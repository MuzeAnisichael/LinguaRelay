...
## M4 asynchronous correction fault gates

`m4-correction-fault-gates.json` is generated by `lingua-relay
correction-benchmark`. It uses a deterministic in-process provider double with
25 ms simulated latency and a separate disconnected-provider scenario. It does
not send caption text over the network and does not claim LLM translation
quality. The report verifies that correction submission remains non-blocking,
all healthy revisions retain their original translation and parent revision,
disconnects emit no false revisions, fast events survive, and the circuit
