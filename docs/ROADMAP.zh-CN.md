...
- [x] 构建包含说话人差异以及会议、视频和背景音乐模拟条件的合法四语测试集；
- [x] 记录 WER/CER、术语命中率、翻译质量和端到端延迟；
- [x] 打包为 Windows 安装程序，首次启动按需下载模型并展示许可证；
- [x] 实现崩溃恢复、提醒式更新策略、隐私说明和威胁建模；
- [x] 发布 `v0.1.0`。

## v0.3：录制与离线工作台（已完成）

- [x] 录制系统输出、指定进程或麦克风，支持开始、暂停、继续、结束和异常片段恢复；
- [x] 导入音频，或从视频中分离音轨；
- [x] 使用词级时间戳、VAD 和更高 Beam 进行后期识别，按标点/时长/字符数生成可读字幕；
- [x] 建立 SQLite 离线项目、播放、波形定位、时间轴和字幕编辑；
- [x] 使用本地翻译与可选 LLM 后期精修；
...
- [ ] 评估可再分发的直译模型与按方向 provider，禁止静默枢轴翻译；
- [ ] 评估 WhisperX 或等价对齐器、说话人分离和长文件批处理恢复；
- [ ] 增加项目重命名、删除、批量导出和原始高保真音轨选项。

详细方案见 [LANGUAGE-EXPANSION.zh-CN.md](LANGUAGE-EXPANSION.zh-CN.md)。
