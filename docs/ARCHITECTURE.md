...
2. **固定语言优先。** 初版由用户选择 `source_language` 和 `target_language`，不运行自动检测。
...
| 系统音频 | PyAudioWPatch / WASAPI loopback | 直接面向 Windows 默认输出设备 | 需要按进程捕获或更低抖动时使用原生 WASAPI/NAudio sidecar |
| 语音识别 | faster-whisper `small`, CPU INT8 | 多语言、CTranslate2 推理、部署成熟 | 根据基准切换 `base`、`turbo`、CUDA 或 whisper.cpp |
| 端点检测 | Silero VAD | 与 faster-whisper 集成，减少静音推理 | 字幕断句体验不足时加入标点/语义端点器 |
| 即时翻译 | OPUS-MT `en-zh` | 单语对模型小、速度快、许可清晰 | 扩展语言对或质量不足时引入其他本地 MT/API |
