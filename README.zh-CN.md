<div align="center">
  ...
  <h1>LinguaRelay</h1>
  <p><strong>实时翻译电脑声音，也把录音与视频变成可编辑字幕。</strong></p>
  <p>本地优先、低延迟，并可选用大模型精修实时或后期译文。</p>
  <p>
  ...
  <p>
    <a href="https://github.com/MuzeAnisichael/LinguaRelay/releases/tag/v0.3.0"><strong>下载 v0.3.0</strong></a>
    · <a href="#快速开始">快速开始</a>
    · <a href="docs/releases/v0.3.0.md">发布说明</a>
  </p>
</div>
...
> [!IMPORTANT]
> v0.3.0 是尚未签名的 Windows x64 Alpha 版本。请只从本仓库下载，使用
...
1. 打开 [GitHub Releases 的 v0.3.0 页面](https://github.com/MuzeAnisichael/LinguaRelay/releases/tag/v0.3.0)，下载 `LinguaRelay-0.3.0-Setup-x64.exe`。
2. 首次启动时，让 LinguaRelay 校验已有模型目录，或选择一种模型方案。安装程序不会在没有提示的情况下静默下载模型。
3. 在托盘菜单中选择源语言、目标语言和系统输出/指定进程/麦克风。播放音频，再把悬浮窗拖动、缩放到合适位置。

不想安装也可以使用 `LinguaRelay-0.3.0-Windows-x64-portable.zip`。安装版和便携版都能复用 v0.1.5 的基础离线模型 ZIP。
...
![LinguaRelay 录制与离线工作台](docs/images/offline-workbench.png)

- 在悬浮窗点击 `●` 开始录制当前系统/进程/麦克风源，使用 `Ⅱ / ▶` 暂停或继续，点击 `■` 结束；暂停空档不会写入最终媒体时间轴。
- 导入已有音频，或从 MP4、MKV、MOV、WebM 等视频中分离音轨；录制和导入项目共用同一条后期流水线。
- 使用 faster-whisper 词级时间戳、VAD 与 Beam Search 重新识别；在 Base、Small、Medium、Large-v3 Turbo、Large-v3 之间选择速度/质量档位。
- 播放音频、查看波形、点击字幕定位，直接编辑开始/结束时间、原文和译文。
- 导出 WAV、FLAC、MP3、WebVTT、SRT、ASS、TXT、CSV 或 JSONL；需要 `MP3 + VTT` 时分别导出两个同名文件。
...
flowchart LR
    ...
    F -. "修正版字幕" .-> E
    B -. "用户主动录制" .-> G["离线项目 / 音视频导入"]
    G --> H["高质量识别 + 翻译 + 时间轴编辑"]
    H --> I["MP3 / VTT / SRT / ASS / 数据导出"]
```
...
- 实时音频默认只在内存中处理；只有主动录制或导入媒体时，音频才写入本地离线项目。字幕历史可以关闭。
...
| [更多语言方案](docs/LANGUAGE-EXPANSION.zh-CN.md) | 扩展语言的模型、许可证和质量门槛 |
