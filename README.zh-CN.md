...
> 当前状态：M1 音频捕获已经完成；实时语音识别和翻译分别属于后续里程碑，项目暂不适合生产使用。
...
- 暂不启用自动语言检测；
- M2 对多语言 `faster-whisper` 模型进行实时识别基准；
...
`lingua-relay demo` 仍可运行悬浮窗演示。修改语言和音频参数前，可以把 `config.example.toml` 复制为 `config.toml`。M1 不会下载语音或翻译模型。
